...
**2b: 先分析 Bug，定位相关代码和测试（必须先于跑测试）**

基于 Step 1 中找到的 spec + 用户描述的 bug 信息：

1. 根据 spec 中涉及的 Requirement/Scenario，定位对应的**源码文件**（参考 `features/[name]/README.md` 中的层导航表）
2. 在测试目录中 grep 相关关键词，定位**对应的测试文件和测试函数**：
   ```bash
   grep -rn "scenario关键词\|requirement关键词\|函数名" <2a 中的测试目录>
   ```
3. 阅读相关测试代码，理解现有 assertion 验证的是什么
4. 记录下"相关测试文件列表"供 2c 使用

**2c: 跑对应测试（非全量）**

只运行 2b 中定位到的相关测试（使用框架提供的文件/函数级过滤），拿到失败输出（或确认全绿但行为仍错）。
...
> 如果 2b 未能定位到任何相关测试 → 该 Scenario **无测试**，直接判定为根因 E（Spec 缺失/测试缺失）→ Step 3C。

**2d: 分诊**

基于 2b 的代码阅读 + 2c 的测试结果，按以下**决策树**逐步分诊（不要凭整体感觉判断，严格按分支走）：
