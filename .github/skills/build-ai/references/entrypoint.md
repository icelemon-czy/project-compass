...
AI 导航入口文件，让 AI 工具在每次对话启动时自动加载 `.ai/` 上下文。

根据用户使用的工具，创建对应文件。内容相同，只是文件名和位置不同。
...
2. **读功能文档** — `.ai/L1-codebase-map/features/[功能名]/README.md`
   - 按需深入：各层 `.md` 文件
3. **读模块规则** — `.ai/L2-rules/[模块名].md`（编码约束 + 合约）
4. **做通用任务时** — 查 `.ai/L1-codebase-map/key-files.md`（任务食谱）
5. **跨模块修改时** — 查 `.ai/L1-codebase-map/module-map.md`（变更联动表）
6. **改底层基础设施时** — 查 `.ai/L1-codebase-map/infrastructure/README.md`
