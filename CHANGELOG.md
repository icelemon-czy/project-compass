...
- CLI worker 从不兼容的 tool-call-level handoff 改为显式 task-level delegation：native hook 阻止 planner 直接写入与 raw Claude command，executor 每个 bounded task 只启动一次 fresh non-persistent Claude session，禁止 resume / continue / session ID，并对成功 task content 去重。
