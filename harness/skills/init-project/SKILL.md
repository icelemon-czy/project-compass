...
### Phase C: 初始化 Compass Harness 上下文 + Spec（自动）

#### Step 5: 确认 Compass Harness 已复制并安装

目标项目必须已经包含 `.compass-harness/INSTALL.md`。如果不存在，停止并要求用户先把 Compass Harness 的 `harness/` 目录复制为项目的 `.compass-harness/`；不要从不明确的位置拼装文件。

读取并执行 `.compass-harness/INSTALL.md`：

- 非破坏地合并根 `AGENTS.md`。
- 直接在已复制的 `.compass-harness/context/` 中填写 L1–L5，已有内容不覆盖，也不创建第二个 context 目录。
- 根 `AGENTS.md` 直接导航到唯一 `.compass-harness/skills/`。
- 默认不安装 Subagent。
...
按 `.compass-harness/INSTALL.md` 验证：根 `AGENTS.md` 保留项目原规则并直接导航到 `.compass-harness/skills/`、没有复制第二套 Skill 或创建 Skill 软链接，也没有生成 Subagent 实例。
...
### 已创建
- [x] 项目脚手架 + 依赖安装
- [x] 根 AGENTS.md + .compass-harness/context/
- [x] 根 AGENTS.md 直接导航到唯一 .compass-harness/skills/
- [x] 基于 Spec 的测试（红灯 → 绿灯）
- [x] 追溯矩阵已更新
