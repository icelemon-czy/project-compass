...
> 用户对代码库有疑问时的统一入口。不做任何代码修改，只读取 `.compass-harness/context/` 文档和源码来回答。
...
- `.compass-harness/context/` 目录已存在且至少有 L1 层
...
1. 读取 `.compass-harness/context/L1-codebase-map/overview.md` — 在功能索引中匹配用户问的功能
2. 匹配到 → 读取 `.compass-harness/context/L1-codebase-map/features/<matched>/README.md` — 获取详细代码位置、层结构、数据流
3. 未匹配 → 读取 `.compass-harness/context/L1-codebase-map/key-files.md` — 尝试从常见任务食谱中找到
...
- Spec：`.compass-harness/context/L3-specs/specs/auth/spec.md`
- 规则：`.compass-harness/context/L2-rules/auth.md`
```

### Step 2B: 架构解释

1. 读取 `.compass-harness/context/L1-codebase-map/overview.md` — 理解整体结构
2. 读取 `.compass-harness/context/L1-codebase-map/architecture.md` — 获取运行时架构、请求生命周期、部署拓扑
3. 读取 `.compass-harness/context/L1-codebase-map/module-map.md` — 理解模块间关系
...
1. 读取 `.compass-harness/context/L1-codebase-map/module-map.md` — **重点看"变更联动表"**，找到被改模块的下游影响
...
3. `.compass-harness/context/L1-codebase-map/features/auth/README.md` — 更新文档
...
1. 读取 `.compass-harness/context/L2-rules/global.md` — 全局规则
2. 如果问题涉及特定模块 → 读取 `.compass-harness/context/L2-rules/<module>.md`
3. 如果问题涉及代码模板 → 读取 `.compass-harness/context/L2-rules/templates.md`
4. 如果问题涉及测试 → 读取 `.compass-harness/context/L2-rules/testing.md`
...
### 规则来源
- 文件：`.compass-harness/context/L2-rules/global.md` L42-L55
- 原文：> [引用原文]
...
1. 读取 `.compass-harness/context/L3-specs/specs/` — 按功能域找到对应的 spec
2. 如果涉及变更历史 → 扫描 `.compass-harness/context/L3-specs/archive/` 下的 `proposal.md`
3. 如果涉及测试 → 读取 `.compass-harness/context/L5-validation/traceability/<domain>.md`
...
### Spec 位置
- 文件：`.compass-harness/context/L3-specs/specs/auth/spec.md`
...
### Step 3: 补充源码搜索（当 .compass-harness/context/ 文档不够时）

如果 Step 2 从 `.compass-harness/context/` 文档中未找到足够信息：

1. 用 `grep -rn` / `find` 在源码中搜索
2. 读取找到的源码文件，提取关键信息
3. **标记为"来自源码直接搜索，非 .compass-harness/context/ 文档"**，提醒用户考虑更新 `.compass-harness/context/`

```
> ⚠️ 以下信息来自源码直接搜索，`.compass-harness/context/` 文档中未记录。建议运行 `/update-ai` 补充。
```
...
3. **相关操作建议**（如适用）：
   ...
   - 如果发现 `.compass-harness/context/` 文档缺失/过期 → 提示 `/update-ai`
   - 如果发现测试缺口 → 提示 `/review-tests`

## 反模式

- ❌ 不读 `.compass-harness/context/` 文档就直接 grep 源码（必须先查 .compass-harness/context/，不够再补搜）
