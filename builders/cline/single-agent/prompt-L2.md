# Cline 构建 L2 编码规则 — Prompt 模板（单 Agent 模式）
...
### Agent 职责划分（单 Agent 模式）

```
主 Agent（Cline）—— 单 Agent，不使用 subagent
├── Phase 4a-4c：从配置文件和 import 分析中提取规则
├── Phase 4d-4e：从代码中搜索错误处理和反模式
├── Phase 4f-4h：提取模板、版本控制、测试规范
├── 填写 global.md → ⏸️ 暂停等待审核
├── Phase 5：逐个处理每个主要模块
│     ├── 【模块: auth】 → 分析 → 创建规则文件 → ⏸️ 暂停等待审核
│     ├── 【模块: orders】 → 分析 → 创建规则文件 → ⏸️ 暂停等待审核
│     └── 【模块: ...】
