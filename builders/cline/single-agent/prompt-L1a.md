# Cline 构建 L1 代码导航文档 — Phase 1-3（发现阶段·单 Agent 模式）

> **使用顺序**：先用本文件，完成后用同目录的 `prompt-L1b.md` 继续。
...
> ☸️ 主 agent 到此为止。每个功能和基础设施组件的深入分析将在 `prompt-L1b.md` 中由主 agent 逐个执行，每完成一个暂停等待人工审核。
