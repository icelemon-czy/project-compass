# Copilot 构建 L3 初始 Spec — Prompt 模板
...
**输出**：能力域列表 + 与 feature 的映射关系。展示给我确认后再继续。
...
**重要**：无论是否有需求文档，spec 都可能不完整 — 业务规则不一定体现在代码或文档中。

1. 展示 system.md 完整内容
2. 展示每个能力域 spec 的完整内容
3. 标注不确定点，提出需要补充的业务问题
4. **等待我确认后再写入文件**
...
4. Agent 读取输入 → 生成能力域列表 → 等你确认
5. Agent 逐域生成 spec → 展示完整内容 → 等你确认
6. 你补充业务规则 → Agent 更新 spec → 完成
```
