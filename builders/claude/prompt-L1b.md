...
> 为 `_handoff.md` 功能清单里的**每个功能**派发一个独立的 subagent。
> subagent 拥有独立上下文，自主阅读代码、分析层次、创建文件。
...
> ❗ 主 agent 快速检查：每个功能至少有 README.md + 分层文件，README 中包含分层导航表。
...
**命名规则**：用项目里真实存在的概念（如 framework / config / plugin-host / logger），禁止用抽象通用词。
...
在 `.ai/L1-codebase-map/infrastructure/` 下直接创建文件：

**README.md**（必须包含以下 section）：
- 「分层导航」表 — 每行对应一个组件文件，写明加载时机和摘要
- 「架构全景」 — 组件之间的关系图 + 功能层如何使用它们
- 「变更影响」表 — 当你改了… / 必须同步改… / 原因
- 「已知陷阱」

**[组件名].md**（每个组件一个文件，文件名 = Step 1 中确定的组件名）：
...
内容格式参考 `.ai/L1-codebase-map/infrastructure/README.md` 中的「组件文件格式参考」部分。
...
```
主 Agent（Claude Code）—— 本对话
├── 读取 _handoff.md（功能清单 + 跨功能模式 + overview 全文）
├── Phase 4：使用 subagents 为每个功能派发独立分析任务（强制，不可选）
│     ├── 【subagent: user-auth】  → 独立 context，自主分析，直接创建文件
│     ├── 【subagent: order-mgmt】 → 独立 context，自主分析，直接创建文件
│     └── 【subagent: ...】
