...
#### 2c: 识别基础设施层
从 Phase 1 的信息中，识别不属于任何单一功能的底层基础设施：

关注以下类别（项目级共享基础设施）：
- **框架基类** — BaseController、BaseService、DI 容器、生命周期管理
- **配置系统** — 配置加载、环境变量映射、配置文件结构
- **插件/扩展机制** — 插件接口、加载器、扩展点、插件生命周期
- **通用工具** — 日志框架、错误处理、序列化、常用 helpers
- **构建/CI 流程** — 构建脚本、CI/CD 流水线、发布流程
- **测试基础设施** — 测试框架配置、通用 mock/fixture、测试 helpers

> 💡 注意：只服务于特定 feature 的 helper 不算基础设施，应归入对应 feature 的层文件。
...
|--------|---------|---------|------|------------|
<!-- 将 Phase 2a 的功能清单完整复制到这里 -->

## 基础设施层（Phase 2c）

<!-- 将 Phase 2c 识别出的基础设施组件表复制到这里。如果没有，写「无」。 -->

## 跨功能通用模式（Phase 2b）

<!-- 将 Phase 2b 识别出的通用模式、任务食谱线索、雷区完整复制到这里 -->
