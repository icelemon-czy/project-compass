...
读取项目代码推断测试框架和模式，引导用户确认，生成或更新 `.compass/context/L2-rules/testing.md`。

## Prerequisites

- `.compass/context/` 目录已存在
- 项目中有代码（不一定已有测试）
...
```bash
cat .compass/context/L2-rules/testing.md 2>/dev/null
```

如不存在，说明复制来的 L2 上下文文件缺失；根据当前项目实际测试框架重建最小 testing.md，并在最终报告中列为待确认，不能创建或依赖第二个模板目录。
...
将最终内容写入 `.compass/context/L2-rules/testing.md`。
...
文件位置：`.compass/context/L2-rules/testing.md`
