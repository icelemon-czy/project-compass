...
- `.compass/context/L3-specs/changes/` 下有至少一个变更（不含 `_change-template`）
...
**有参数** → 直接读取 `.compass/context/L3-specs/changes/<name>/proposal.md`，如不存在则报错。
...
4. 读取 `.compass/context/L4-session/active-session.md` — 是否有上次中断的进度
...
参考 `.compass/context/L3-specs/changes/_change-template/tasks.md`：
...
1. 读取 `.compass/context/L2-rules/global.md` + 相关模块规则 — 遵守编码约束
...
2. 更新 `.compass/context/L5-validation/traceability/<domain>.md` — 新增/修改的 Scenario 标为 ✅ verified
