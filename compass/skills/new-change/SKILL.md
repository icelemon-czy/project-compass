...
- `.compass/context/` 目录已存在且有 L1 + L3 + L5 结构
...
1. 读取 `.compass/context/L1-codebase-map/overview.md` — 项目功能索引
2. 读取 `.compass/context/L3-specs/specs/system.md` — 系统级需求
...
3. 根据变更描述，定位涉及的功能和模块：
   - `.compass/context/L1-codebase-map/features/[name]/README.md`
   - `.compass/context/L2-rules/[module].md`
   - `.compass/context/L3-specs/specs/<domain>/spec.md`（如已存在）
...
```
.compass/context/L3-specs/changes/<name>/
├── proposal.md
├── specs/         (delta specs, Step 4 生成)
└── tasks.md       (Step 5 生成)
```

参考 `.compass/context/L3-specs/changes/_change-template/proposal.md`，填写：
...
参考 `.compass/context/L3-specs/changes/_change-template/tasks.md`，根据 proposal + delta spec 生成执行步骤：
...
读取 `.compass/context/L2-rules/testing.md`（如存在）— 遵守项目测试规范。
...
1. **强制注入 L2 规则（写任何新代码前）**：完整读取 `.compass/context/L2-rules/global.md`、`.compass/context/L2-rules/testing.md`、以及此变更涉及的每个模块的 `L2-rules/<module>.md`。在输出中明列本次要遵守的关键规则（3-5 条）。不允许跳过
2. 跨模块修改 → 先查 `.compass/context/L1-codebase-map/module-map.md` 变更联动表
3. 创建新文件 → 查 `.compass/context/L2-rules/templates.md`
