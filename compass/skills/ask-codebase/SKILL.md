...
> 用户对代码库有疑问时的统一入口。不做任何代码修改，只读取 `.compass/context/` 文档和源码来回答。
...
- `.compass/context/` 目录已存在且至少有 L1 层
...
1. 读取 `.compass/context/L1-codebase-map/overview.md` — 在功能索引中匹配用户问的功能
2. 匹配到 → 读取 `.compass/context/L1-codebase-map/features/<matched>/README.md` — 获取详细代码位置、层结构、数据流
3. 未匹配 → 读取 `.compass/context/L1-codebase-map/key-files.md` — 尝试从常见任务食谱中找到
...
- Spec：`.compass/context/L3-specs/specs/auth/spec.md`
- 规则：`.compass/context/L2-rules/auth.md`
```

### Step 2B: 架构解释

1. 读取 `.compass/context/L1-codebase-map/overview.md` — 理解整体结构
2. 读取 `.compass/context/L1-codebase-map/architecture.md` — 获取运行时架构、请求生命周期、部署拓扑
3. 读取 `.compass/context/L1-codebase-map/module-map.md` — 理解模块间关系
...
1. 读取 `.compass/context/L1-codebase-map/module-map.md` — **重点看"变更联动表"**，找到被改模块的下游影响
...
3. `.compass/context/L1-codebase-map/features/auth/README.md` — 更新文档
...
1. 读取 `.compass/context/L2-rules/global.md` — 全局规则
2. 如果问题涉及特定模块 → 读取 `.compass/context/L2-rules/<module>.md`
3. 如果问题涉及代码模板 → 读取 `.compass/context/L2-rules/templates.md`
4. 如果问题涉及测试 → 读取 `.compass/context/L2-rules/testing.md`
...
### 规则来源
- 文件：`.compass/context/L2-rules/global.md` L42-L55
- 原文：> [引用原文]
...
1. 读取 `.compass/context/L3-specs/specs/` — 按功能域找到对应的 spec
2. 如果涉及变更历史 → 扫描 `.compass/context/L3-specs/archive/` 下的 `proposal.md`
3. 如果涉及测试 → 读取 `.compass/context/L5-validation/traceability/<domain>.md`
...
### Spec 位置
- 文件：`.compass/context/L3-specs/specs/auth/spec.md`
...
- ❌ 不读 `.compass/context/` 文档就直接 grep 源码（必须先查 .compass/context/，不够再补搜）
