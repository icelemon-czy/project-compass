...
汇总 `.compass/context/L3-specs/` 下所有变更的状态，给出全局视图。

## Prerequisites

- `.compass/context/L3-specs/` 目录已存在
...
# 如果用户要看全量（含归档）
ls .compass/context/L3-specs/archive/ 2>/dev/null
```
...
- ⚠️ 变更的 tasks.md 中有超过 **7 天**未推进的 checkbox（依据：`git log --format='%ai' -1 -- .compass/context/L3-specs/changes/<name>/`，如最后修改距今 > 7 天则告警）
