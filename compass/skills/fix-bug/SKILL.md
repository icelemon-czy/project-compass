...
P1 — review 打回（阻塞归档，最高优先）
   来源：.compass/context/L5-validation/reports/ 下最近报告中结论为 ❌ 打回 的条目
...
1. `ls .compass/context/L3-specs/changes/` — 是否有 `review-failed` 或 `implementing` 状态的变更涉及相关功能？
...
1. 读取 `.compass/context/L1-codebase-map/overview.md` — 匹配涉及的功能
2. 读取对应的 `features/[name]/README.md` — 理解数据流和相关代码
3. 读取 `.compass/context/L3-specs/specs/` 下对应能力域的 `spec.md` — 找到相关 Requirement + Scenario
...
读取 `.compass/context/L2-rules/testing.md` — 获取：
...
1. 读取 `.compass/context/L2-rules/global.md` + 相关模块规则
2. 读取 `.compass/context/L1-codebase-map/features/<相关功能>/README.md` — 定位 bug 代码位置（层导航表、数据流）
3. 修复代码
4. 跑测试，确认由红转绿
5. 跳到 Step 5
...
1. 在 `.compass/context/L3-specs/changes/` 下创建或复用 fix 变更（如 `fix-empty-password-validation`）
