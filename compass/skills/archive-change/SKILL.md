...
**有参数** → 直接读取 `.compass/context/L3-specs/changes/<name>/proposal.md`，确认状态为 `approved`。
...
1. 读取 `.compass/context/L3-specs/changes/<name>/proposal.md` — 确认状态和变更目的
2. 读取 `.compass/context/L3-specs/changes/<name>/specs/` 下所有 delta spec — 了解新增/修改/删除了哪些 Requirement
3. 读取 `.compass/context/L3-specs/changes/<name>/tasks.md`（如存在）— 确认任务完成状态
...
2. 读取主 spec（`.compass/context/L3-specs/specs/<cap>/spec.md`，如存在）
...
读取 `.compass/context/L5-validation/traceability/<domain>.md`（对每个涉及的能力域），然后更新：
...
3. 如果 `.compass/context/L4-session/active-session.md` 指向该变更 → 清除引用

```bash
mkdir -p .compass/context/L3-specs/archive/
mv .compass/context/L3-specs/changes/<name>/ .compass/context/L3-specs/archive/<name>/
```
