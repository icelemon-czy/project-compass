...
- `.compass/context/` 目录已存在且有 L3 + L5 结构
- 项目有测试代码
...
读取 `.compass/context/L2-rules/testing.md` — 获取以下信息并记录到工作区：
...
ls .compass/context/L3-specs/changes/ | grep -v _change-template
# 对每个变更，读取 proposal.md 中的 status 字段，保留 status == pending-review 的

# 优先级 2：L3 已有 spec（archived 能力域），按能力域字母顺序
ls .compass/context/L3-specs/specs/ | grep -v _capability-template | grep -v system.md
```

**队列规则**：
1. **P1 优先**：所有 `pending-review` 状态的变更，按创建时间（proposal.md 的 `created` 字段或文件 mtime）升序
2. **P2 兜底**：当 P1 为空时，按能力域字母顺序遍历 `.compass/context/L3-specs/specs/` 中的所有 spec
...
读取 `.compass/context/L5-validation/reports/` 下最近的 review 报告（如存在）。如果 P2 队列（spec 类目标）最近一次审查时间 < 配置的冷却期（默认 7 天）且无代码变更 → 跳到下一项。

> 冷却规则：可在 `.compass/context/L2-rules/global.md` 的 `review-tests.cooldown-days` 字段配置。
...
- 目标是 change → `.compass/context/L3-specs/changes/<change-name>/specs/*/spec.md`（delta spec）
- 目标是 spec-domain → `.compass/context/L3-specs/specs/<domain>/spec.md`
...
> **项目自定义反模式**（第 8+ 条）：读取 `.compass/context/L2-rules/testing.md` 的"自定义反模式"区段，逐条追加检查。前 7 条是底线，不可删除。
...
> 以下状态回流仅适用于 **change** 目标。若本次审查目标是 `spec-domain`，则不改 proposal.md 状态，只更新报告和 `.compass/context/L4-session/active-session.md`。
...
更新 `.compass/context/L4-session/active-session.md`，记录审查结果和下一步动作。

**生成审查报告文件**：将完整报告写入 `.compass/context/L5-validation/reports/review-<target-name>-<YYYYMMDD>.md`，供冷却期判断（Step 0d）复用。
