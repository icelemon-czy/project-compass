---
name: update-ai
description: "Update existing .compass/context project context when code changes. Use when: refresh .compass/context docs, code changed need to update ai context, sync .compass/context with code, 更新AI上下文, 刷新.compass/context文档, update ai docs, new feature added update docs"
---

# Update .compass/context Project Context

Updates an existing `.compass/context/` directory to reflect code changes.
...
- `.compass/context/` directory already exists with L1-L4 structure
- Use `/build-ai` if no `.compass/context/` exists yet

## Procedure

### Step 1: Assess Current State

Read the existing `.compass/context/` structure to understand what's documented:

```bash
# What .compass/context docs exist?
find .compass/context -name "*.md" | sort

# Current overview
cat .compass/context/L1-codebase-map/overview.md

# Current feature list
ls .compass/context/L1-codebase-map/features/

# Last session state
cat .compass/context/L4-session/active-session.md
```
...
git diff --name-status HEAD~30 -- . ':!.compass'

# Current project structure
find . -maxdepth 3 -not -path '*/node_modules/*' -not -path '*/.git/*' -not -path '*/dist/*' -not -path '*/__pycache__/*' -not -path '*/venv/*' -not -path '*/.venv/*' -not -path '*/build/*' -not -path '*/target/*' -not -path '*/.compass/*' | head -120 | sort
...
4. Check `key-files.md` for references to deleted feature — `grep -n "<feature-name>" .compass/context/L1-codebase-map/key-files.md`，命中的行删除或标注"已移除"
...
1. **overview.md** — `ls .compass/context/L1-codebase-map/features/` 的目录列表 vs overview.md 中的 feature 列表，缺的补、多的删
...
Update `.compass/context/L4-session/active-session.md`:

```markdown
- **时间**: [now]
- **对话主题**: .compass/context 文档更新
