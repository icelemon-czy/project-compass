...
- 按源码仓 `doc/install_instruction.md` 的 Skill inventory 和 copy 规则安装全部 Compass Skill，不创建软链接，也不写入 `~/.cursor/skills/`。
- 不修改用户自建其他 Skill。
...
2. Skill 不含 ownership marker；按 Skill inventory 列出 `.cursor/skills/<skill>/`，只有用户逐项明确确认后才删除。
