...
> 本文件由 `.compass/INSTALL.md` 调用，只负责 Cursor 的项目入口、可选 Subagent 和可选 CLI worker hook。本版本不安装 Compass Skill。
...
- 不安装 Compass Skill，不创建 `.cursor/skills/` 来承载 Compass Skill。用户自建 Skill 全部保留。
- 不修改 Cursor user settings，也不写入 `~/.cursor/hooks.json`。

## Step 1：安装 Cursor project instructions

1. 检查项目是否已有根 `AGENTS.md`、`.cursor/agents/` 和 `.cursor/hooks.json`。
...
报告 `Skills：none`。不要复制 Skill，不要新建 `.cursor/skills/`。
...
- [ ] 没有为 Compass 创建或写入 `.cursor/skills/`。
- [ ] 未明确选择时没有生成 Subagent。
